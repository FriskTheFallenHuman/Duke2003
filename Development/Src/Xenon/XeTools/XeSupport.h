...
typedef gcroot<XeTools::XenonConsole^> TARGET;
typedef map<wstring, TARGET> TargetMap;
typedef map<TARGETHANDLE, TARGET> TargetHandleMap;
...
private:
	...
	TargetHandleMap CachedConnections;

	/**  Holds connection names to prevent from enumerating duplicates. */
...
private:
	 ...
	 */
	TARGET GetTarget(const wchar_t *Name, TARGETHANDLE &OutHandle);

	static DWORD WINAPI SyncThreadMain(void *lpParameter);
	static bool MakeDirectoryHierarchy(XDevkit::XboxConsole ^Target, System::String ^DirectoryHierarchy);
