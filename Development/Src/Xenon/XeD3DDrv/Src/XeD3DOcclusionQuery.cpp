...
{
	IDirect3DQuery9* NewQuery = NULL;
	VERIFYD3DRESULT(GDirect3DDevice->CreateQuery(D3DQUERYTYPE_OCCLUSION, &NewQuery));
	FOcclusionQueryRHIRef OcclusionQuery(new FXeOcclusionQuery(NewQuery));
...
UBOOL RHIGetOcclusionQueryResult(FOcclusionQueryRHIParamRef OcclusionQuery,DWORD& OutNumPixels,UBOOL bWait)
{
	...
	HRESULT Result = OcclusionQuery->Query->GetData( &OutNumPixels, sizeof(OutNumPixels), 0 );
	...
	if ( bWait && (Result != S_OK) )
	{
		...
		do 
		{
			Result = OcclusionQuery->Query->GetData( &OutNumPixels, sizeof(OutNumPixels), D3DGETDATA_FLUSH );
