...
D3DFORMAT XeGetRenderTargetFormat(BYTE Format)
{
	...
	// override with render target usable format
    switch( Format )
	{
	case PF_FloatRGB:
	...
	case PF_R32F:
		...
		break;
	case PF_G8:
		// can only render to 32 bit formats on 360
...
/**
* Calculates the exponent bias applied when rendering to the given surface format.
* @return bias to apply when rendering to the given render target format
*/
INT XeGetSurfaceColorExpBias(D3DFORMAT SurfaceD3DFormat, D3DFORMAT TextureD3DFormat)
{
	if (SurfaceD3DFormat == D3DFMT_A16B16G16R16_EDRAM && TextureD3DFormat == D3DFMT_A16B16G16R16)
	{
		// D3DFMT_A16B16G16R16_EDRAM has a range of (-32, 32), and D3DFMT_A16B16G16R16 has a range of [0, 1]
		// Scale up pixel shader outputs from [0, 1] to [0, 32] to make better use of the render target range
		return 5;
	}
	return 0;
}
...
{
	//@todo - change XeGetRenderTargetColorExpBias to only control the resolve bias, and remove write biases from shaders, use XeGetSurfaceColorExpBias instead
	...
	else if (SurfaceD3DFormat == D3DFMT_A16B16G16R16_EDRAM && TextureD3DFormat == D3DFMT_A16B16G16R16)
	{
		// D3DFMT_A16B16G16R16_EDRAM has a range of (-32, 32), and D3DFMT_A16B16G16R16 has a range of [0, 1]
		// Using an exponent of -5 to convert [0, 32] to [0, 1], the negative range will be lost
		return 5;
	}
...
{
	return SurfaceRHI.ResolveTargetTexture2D;
}
	...
	)
{
	...
	else
	{
		...
		// offset to start of available edram region		
		SurfaceParameters.Base = XeEDRAMOffset(UsageStr,SurfaceSize);
		SurfaceParameters.ColorExpBias = XeGetSurfaceColorExpBias(SurfaceD3DFormat, TextureD3DFormat);
...
*/
void RHIReadSurfaceData(FSurfaceRHIParamRef InSurface,UINT MinX,UINT MinY,UINT MaxX,UINT MaxY,TArray<BYTE>& OutData, FReadSurfaceDataFlags InFlags)
{
	...
	check(SurfaceDesc.Format == D3DFMT_A8R8G8B8 || SurfaceDesc.Format == MAKESRGBFMT(D3DFMT_A8R8G8B8));
...
{
	FTexture2DRHIParamRef ResolveTarget2D = ResolveParams.ResolveTarget ? ResolveParams.ResolveTarget : SourceSurface.ResolveTargetTexture2D;

	if(ResolveTarget2D || IsValidRef(SourceSurface.ResolveTargetTextureCube) || ResolveParams.ArrayTarget)
	{
		...
		if( ResolveParams.Rect.X1 >= 0 && ResolveParams.Rect.X2 >= 0 && ResolveParams.Rect.Y1 >= 0 && ResolveParams.Rect.Y2 >= 0 )
		{
			...
			SrcRect.x1 = Max(Align(ResolveParams.Rect.X1 - (D3DRESOLVEALIGNMENT - 1), D3DRESOLVEALIGNMENT), 0);
			SrcRect.y1 = Max(Align(ResolveParams.Rect.Y1 - (D3DRESOLVEALIGNMENT - 1), D3DRESOLVEALIGNMENT), 0);
            // rect dimensions must be aligned to 32
			INT AlignedSizeX = ResolveParams.Rect.X2 - SrcRect.x1;
			INT AlignedSizeY = ResolveParams.Rect.Y2 - SrcRect.y1;

			// For texture arrays the caller needs to take tile size into account.
			if(!ResolveParams.ArrayTarget)
			{
				// X360 Resolve() restriction unless destination or source is smaller
				AlignedSizeX = Align(AlignedSizeX, 32);
				AlignedSizeY = Align(AlignedSizeY, 32);
			}
		...
		else
		{
			...
			// array resolve texture
			if(ResolveParams.ArrayTarget)
			{
				IDirect3DArrayTexture9* D3DTextureArray = (IDirect3DArrayTexture9*)(ResolveParams.ArrayTarget->Resource);
	...
	{
		appErrorf(TEXT("Invalid resolve target"));
	}
}
...
UINT XeEDRAMOffset( const TCHAR* Usage, UINT Size )
{
	...
	const DWORD DefaultDepthSize = XGSurfaceSize( GScreenWidth, GScreenHeight, D3DFMT_D24S8, D3DMULTISAMPLE_NONE );
	const DWORD DefaultColorSize = XGSurfaceSize( GScreenWidth, GScreenHeight, D3DFMT_A8R8G8B8, D3DMULTISAMPLE_NONE );
	const DWORD SmallDepthSize = XGSurfaceSize( GScreenWidth/2, GScreenHeight/2, D3DFMT_D24S8, D3DMULTISAMPLE_NONE );
	const DWORD SmallColorSize = XGSurfaceSize( GScreenWidth/2, GScreenHeight/2, D3DFMT_A8R8G8B8, D3DMULTISAMPLE_NONE );
	...
	//	+---------------+---------------+--------+------+
	//
	//	SceneColor:		SceneColor, DefaultBB, AuxColor, LightAttenuation0, DefaultColorRaw
	...
	if( appStricmp(Usage,TEXT("DefaultDepth"))==0 ||
		appStricmp(Usage,TEXT("DominantShadowDepthZ"))==0 ||
		appStricmp(Usage,TEXT("DoFBlurBuffer"))==0 )
	{
	...
	else if( appStricmp(Usage,TEXT("SmallDepth"))==0 )
	{
		...
		EDRamOffset = DefaultDepthSize + DefaultColorSize;
			 ...
			 appStricmp(Usage,TEXT("PreshadowCache"))==0 ||
	...
	else if( appStricmp(Usage,TEXT("FilterColor1"))==0 ||
			 appStricmp(Usage,TEXT("FilterColor2"))==0 ||
			 appStricmp(Usage,TEXT("FilterColor3"))==0 ||
			 appStricmp(Usage,TEXT("FogFrontfacesIntegralAccumulationRT"))==0 ||
			 appStricmp(Usage,TEXT("FogBackfacesIntegralAccumulationRT"))==0 ||
			 appStricmp(Usage,TEXT("TranslucencyBuffer"))==0 ||
			 appStricmp(Usage,TEXT("HalfResPostProcess"))==0 ||
			 appStricmp(Usage,TEXT("TranslucencyShadowDepthZ"))==0 ||
			 appStricmp(Usage,TEXT("AmbientOcclusion"))==0 ||
			 appStricmp(Usage,TEXT("LUTBlend"))==0 ||
	...
	else if( appStricmp(Usage, TEXT("GFxTempColor"))== 0 )
	{
		EDRamOffset = DefaultDepthSize;
	}
	else if( appStricmp(Usage, TEXT("GFxTempDepth")) == 0)
	{
		EDRamOffset = 0;
	}
	else
	...
	checkf(EDRamOffset + Size < GPU_EDRAM_TILES, TEXT("Render target %s at EDRAM offset %d with %d tiles doesn't fit in EDRAM!"), Usage, EDRamOffset, Size);
...
{
	const DWORD DepthBufferSize = XGHierarchicalZSize( GScreenWidth, GScreenHeight, D3DMULTISAMPLE_NONE );
	const DWORD SmallDepthBufferSize = XGHierarchicalZSize( GScreenWidth / 2, GScreenHeight / 2, D3DMULTISAMPLE_NONE );
	...
	else if( appStricmp(Usage, TEXT("TranslucencyShadowDepthZ")) == 0 )
	{
		// Co-exist with SceneDepth and SmallDepth
		HiZOffset = DepthBufferSize + SmallDepthBufferSize;
	}
