...
enum EXeD3DRHIStats
{
	...
	STAT_VertexDeclarationCacheBytes
};
...
const static FVector GDefaultGammaRampParameters = FVector(0.127f, 2.07f, 0.95f);

/**
 * Enables or disables a gamma ramp that compensates for Xbox's default gamma ramp, which makes dark too dark.
 * the "Custom" parameters have been carefully tweaked to look similar to PC (might get tweaked further, even the formula might change)
 * (0,0,0) allows to use the GameDS provided table.
 */
extern void XeSetCorrectiveGammaRamp(UBOOL On, FVector Custom = GDefaultGammaRampParameters);
