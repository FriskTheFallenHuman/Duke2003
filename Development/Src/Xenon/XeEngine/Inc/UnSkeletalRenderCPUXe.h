/*=============================================================================
	UnStaticMeshRenderCPUXe.h: Xbox specific CPU skin optimizations
...
FORCEINLINE void GetPlaneDotsXbox(INT NumTriangles,const FVector4& LightPosition,
	FFinalSkinVertex* RESTRICT Vertices,WORD* RESTRICT Indices,
	FLOAT* RESTRICT PlaneDots)
{
	...
	for (INT TriangleIndex = 0; TriangleIndex < NumTriangles; TriangleIndex++)
	{
		...
		VectorRegister V1 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 0]].Position );
		VectorRegister V2 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 1]].Position );
		VectorRegister V3 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 2]].Position );
		// Get the face normal
	...
	for(INT TriangleIndex = 0;TriangleIndex < NumTriangles;TriangleIndex++)
	{
		const FVector& V1 = Vertices[Indices[TriangleIndex * 3 + 0]].Position;
		const FVector& V2 = Vertices[Indices[TriangleIndex * 3 + 1]].Position;
		const FVector& V3 = Vertices[Indices[TriangleIndex * 3 + 2]].Position;
