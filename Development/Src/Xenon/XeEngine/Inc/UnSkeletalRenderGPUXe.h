/*=============================================================================
	UnStaticMeshRenderGPUXe.h: Xbox specific GPU skin optimizations
...
FORCEINLINE void GetPlaneDotsXbox(INT NumTriangles,const FVector4& LightPosition,
	FVector* RESTRICT Vertices,WORD* RESTRICT Indices,FLOAT* RESTRICT PlaneDots)
{
	...
	for (INT TriangleIndex = 0; TriangleIndex < NumTriangles; TriangleIndex++)
	{
		...
		VectorRegister V1 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 0]] );
		VectorRegister V2 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 1]] );
		VectorRegister V3 = VectorLoadFloat3( &Vertices[Indices[TriangleIndex * 3 + 2]] );
		// Get the face normal
	...
	for(INT TriangleIndex = 0;TriangleIndex < NumTriangles;TriangleIndex++)
	{
		const FVector& V1 = Vertices[Indices[TriangleIndex * 3 + 0]];
		const FVector& V2 = Vertices[Indices[TriangleIndex * 3 + 1]];
		const FVector& V3 = Vertices[Indices[TriangleIndex * 3 + 2]];
