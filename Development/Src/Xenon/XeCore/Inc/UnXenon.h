...
/**
 * Make sure we are compiling against the XDK we are expecting to.
 */
const INT REQUIRED_XDK_VERSION = 20871;
checkAtCompileTime(_XDK_VER == REQUIRED_XDK_VERSION, XDK_VERSION_DoesNotMatchRequiredVersion);
...
#pragma warning(disable : 4826) /* This warning message indicates that conversion from one type to another is sign-extended. This may cause unexpected run-time behavior. */
...
/** 360 memcpy optimized for big blocks */
inline void appBigBlockMemcpy( void* Dest, const void* Src, INT Count ) { XMemCpy( Dest, Src, Count ); }
/** 360 memcpy optimized for big blocks that avoid L2 cache pollution */
inline void appStreamingMemcpy( void* Dest, const void* Src, INT Count ) { XMemCpyStreaming( Dest, Src, Count ); }
...
{
	enum { MAX_CMDLINE_LENGTH=MAX_LAUNCH_DATA_SIZE };
 ...
 */
extern UBOOL XeIsHDDCachingEnabled();
 ...
 */
extern void XeControlHDDCaching( UBOOL bEnableBackgroundCaching );
 ...
 */
extern void XeLogHDDCacheStatus( UBOOL bViewHDDCacheFiles );
...
#define FILECACHE_GAMESIZE			(QWORD(10) * QWORD(1024) * QWORD(1024)) // Amount of cache space reserved for dynamic game resources (stats, etc)
#define FILECACHE_CACHESIZE			((QWORD(2) * QWORD(1024) * QWORD(1024) * QWORD(1024)) - (FILECACHE_GAMESIZE))  // Amount of cache space reserved for auto disc caching
 ...
 * 2 = Stats sender 
 * 3 = Rendering thread
 * 4 = Thread pool & XHV & Audio & Background logging when not FR & hipri1
 * 5 = Thread pool & Async i/o & hipri2
 * 5 = Background file caching thread
 */
#define ASYNCIO_HWTHREAD			5
#define XHV_HWTHREAD				4
#define PHYSICS_HWTHREAD			1
#define RENDERING_HWTHREAD			3
#define AUDIO_HWTHREAD				XboxThread4
...
#define HIPRI_THREAD_POOL_COUNT		2
#define HIPRI_THREAD_POOL_HWTHREAD	THREAD_POOL_HWTHREAD

#define STATS_LISTENER_HWTHREAD		1
#define STATS_SENDER_HWTHREAD		2
#define ASYNC_INPUT_HWTHREAD		1
#define BACKGROUND_LOGGING_HWTHREAD	4
#define FILECACHE_HWTHREAD			5


// Please put new items above the thread info so it's always at the bottom for easy finding
