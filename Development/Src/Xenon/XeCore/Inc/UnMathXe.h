...
{
	return __vperm( __lvx(Ptr, 0), __lvx(Ptr, 15), __lvsl(Ptr,0) );
}
...
{
	return __vperm( __lvx(Ptr, 0), __lvx(Ptr, 11), __lvsl(Ptr,0) );
}
...
{
	return __vspltw( __vperm( __lvx(Ptr, 0), __lvx(Ptr, 3), __lvsl(Ptr,0) ), 0 );
}
 ...
 */
#define VectorStore( Vec, Ptr )					__storeunalignedvector( (Vec), (Ptr) )
 ...
 */
#define VectorCompareEQ( Vec1, Vec2 )			__vcmpeqfp( (Vec1), (Vec2) )
 ...
 * @return		VectorRegister( Vec1.x != Vec2.x ? 0xFFFFFFFF : 0, same for yzw )
 */
#define VectorCompareNE( Vec1, Vec2 )			__vxor(VectorCompareEQ( (Vec1), (Vec2) ), __vspltisw(-1))
 ...
 * @return		VectorRegister( Vec1.x > Vec2.x ? 0xFFFFFFFF : 0, same for yzw )
 */
#define VectorCompareGT( Vec1, Vec2 )			__vcmpgtfp( (Vec1), (Vec2) )
 ...
 * @return		VectorRegister( Vec1.x >= Vec2.x ? 0xFFFFFFFF : 0, same for yzw )
 */
#define VectorCompareGE( Vec1, Vec2 )			__vcmpgefp( (Vec1), (Vec2) )
 ...
 */
#define VectorSelect( Mask, Vec1, Vec2 )			__vsel( (Vec2), (Vec1), (Mask) )
 ...
 */
#define VectorBitwiseOr(Vec1, Vec2)	__vor((Vec1), (Vec2))
 ...
 * @return		VectorRegister( for each bit i: Vec1[i] & Vec2[i] )
 */
#define VectorBitwiseAnd(Vec1, Vec2) __vand((Vec1), (Vec2))
 ...
 * @return		VectorRegister( for each bit i: Vec1[i] ^ Vec2[i] )
 */
#define VectorBitwiseXor(Vec1, Vec2) __vxor((Vec1), (Vec2))
...
*/
#define VectorReciprocalSqrt(Vec)		__vrsqrtefp( (Vec) )
 ...
 */
#define VectorReciprocal(Vec)			__vrefp( (Vec) )
...
{
	return XMVectorReciprocalSqrt( Vec );
}
...
*/
#define VectorSet_W0( Vec )		__vsel( (Vec), VectorZero(), XBOXALTIVEC_SELECT_MASK )
/**
...
*/
#define VectorSet_W1( Vec )		__vsel( (Vec), VectorOne(), XBOXALTIVEC_SELECT_MASK )
 ...
 */
#define VectorMergeVecXYZ_VecW( VecXYZ, VecW )	VectorSelect( XBOXALTIVEC_SELECT_MASK, VecW, VecXYZ )
...
{
	XMVECTOR VR = Vec;
	return XMVector4IsNaN(VR) || XMVector4IsInfinite(VR);
}
