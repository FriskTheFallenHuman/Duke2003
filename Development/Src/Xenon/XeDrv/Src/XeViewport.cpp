...
,	Client(InClient)
{
	...
	// set the fixed aspect ratio for the viewport 
	FixedAspectRatio = VideoMode.fIsWideScreen ? AspectRatio16x9 :
		...
		for( INT ControllerIndex = 0;ControllerIndex < MAX_NUM_CONTROLLERS; ControllerIndex++ )
		{
			...
			if (Packet.ControllerPresent[ControllerIndex])
			{
...

				{
					...
					// Support the "scene it" big button controller
					PassKeyStateToViewportClient( ButtonIndex++, ControllerIndex, KEY_XboxTypeS_BigButton	, CurrentInput.Gamepad.wButtons & XINPUT_GAMEPAD_BIGBUTTON	? DIGITAL_PRESSED : DIGITAL_RELEASED, CurrentTime );
					...
					ViewportClient->InputAxis( this, ControllerIndex, KEY_XboxTypeS_LeftTriggerAxis,	CurrentInput.Gamepad.bLeftTrigger / 255.f,	DeltaTime, TRUE );
					ViewportClient->InputAxis( this, ControllerIndex, KEY_XboxTypeS_RightTriggerAxis,	CurrentInput.Gamepad.bRightTrigger / 255.f,	DeltaTime, TRUE );
	...
	if( Client && ViewportClient->RequiresKeyboardInput() )
	{
		...
		while( XInputGetKeystroke( XUSER_INDEX_ANY, XINPUT_FLAG_KEYBOARD, &KeyStroke ) == ERROR_SUCCESS ) //@hack xenon: can't use "any device" flag as joypad input gets treated as keyboard input then
		{
			...
			if( Key )
			{
				...
				{
					ViewportClient->InputKey( this,0,*Key,KeyStates[KeyStroke.VirtualKey] ? IE_Repeat : IE_Pressed, 1.f, FALSE );
				...
				{
					ViewportClient->InputKey( this,0,*Key,IE_Released, 1.f, FALSE );
