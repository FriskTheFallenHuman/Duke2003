...
/** Determines whether dpad input allows cardinal or diagonal inputs */
UBOOL FAsyncInputPoller::bWantsCardinalDpadOnly = FALSE;


FAsyncInputPoller::FAsyncInputPoller(void) :
...
void FAsyncInputPoller::StartThread()
{
	if( RefCount++ == 0 )
	{
		...
		GConfig->GetBool(TEXT("AsyncInputThread"),TEXT("bWantsCardinalDpadOnly"),FAsyncInputPoller::bWantsCardinalDpadOnly,GInputIni);
	}
}
...
UBOOL FAsyncInputPoller::GetInterpolatedState(FAsyncInputPacket &OutPacket)
{
	...
	while(AsyncInputPoller->Packets.Num())
	{
		...
		for( UINT ControllerIndex = 0;ControllerIndex < FXenonViewport::MAX_NUM_CONTROLLERS; ControllerIndex++ )
		{
			...
			// This code will invalidate any dpad input that happens on a diagonal if requested by the game
			// This is because the async thread will perform two inputs in the same frame instead of
			// treating them as a single diagonal input
			if (bWantsCardinalDpadOnly)
			{
				INT DpadInputCount = 0;
				DpadInputCount += (OutPacket.State[ControllerIndex].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP) ? 1 : 0;
				DpadInputCount += (OutPacket.State[ControllerIndex].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN) ? 1 : 0;
				DpadInputCount += (OutPacket.State[ControllerIndex].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT) ? 1 : 0;
				DpadInputCount += (OutPacket.State[ControllerIndex].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT) ? 1 : 0;
				// Zero the input if we have more than 1 direction
				if (DpadInputCount > 1)
				{
					OutPacket.State[ControllerIndex].Gamepad.wButtons &= ~(XINPUT_GAMEPAD_DPAD_UP |
