...
UBOOL UXenonClient::Exec(const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	else if( ParseCommand(&Cmd,TEXT("GammaRamp")) )
	{
#if USE_XeD3D_RHI
		static FVector CustomCurve = GDefaultGammaRampParameters;

		FString Parameter0(ParseToken(Cmd, 0));
		FString Parameter1(ParseToken(Cmd, 0));
		FString Parameter2(ParseToken(Cmd, 0));

		if(Parameter0.Len())
		{
			if(Parameter1.Len() && Parameter2.Len())
			{
				CustomCurve.X = appAtof(*Parameter0);
				CustomCurve.Y = appAtof(*Parameter1);
				CustomCurve.Z = appAtof(*Parameter2);
			}
			else
			{
				GUseCorrectiveGammaRamp = appAtoi(*Parameter0) != 0;
			}
		}
		else
		{
			Ar.Logf(TEXT("GammaRamp <a> <b> <c> to set custom values (0.127 2.07 0.95 works well, 0 0 0 to set the old one)"));
			Ar.Logf(TEXT("          a = x split between nonlinear and linear ramp 0..1"));
			Ar.Logf(TEXT("          b = Incline(derivation) at x = 0 affecting x = 0 to a"));
			Ar.Logf(TEXT("          c = Incline(derivation) for the linear part from x = a to 1"));
			Ar.Logf(TEXT("GammaRamp <0=off/1=on> to toggle the usage of the Xbox360 gamma ramp"));
			Ar.Logf(TEXT("GammaRamp to get this help"));
		}
		Ar.Logf(TEXT("GammaRamp = %s    Custom = %g %g %g"), GUseCorrectiveGammaRamp ? TEXT("on") : TEXT("off"), CustomCurve.X, CustomCurve.Y, CustomCurve.Z);

		ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
			SetGammaRamp,
			UBOOL, bUseCorrectiveGammaRamp, GUseCorrectiveGammaRamp,
			FVector, Custom, CustomCurve,
		{
			XeSetCorrectiveGammaRamp(bUseCorrectiveGammaRamp, Custom);
		});
#endif
		return TRUE;
	}
	...
	else if( ParseCommand(&Cmd, TEXT("HDDCache")) )
	{
