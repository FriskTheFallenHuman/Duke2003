...
#define	SPURS_PRINTSERVICE_STACK_SIZE	(1024 * 64)
...
spu_printf_handler_entry(uint64_t arg)
{
	...
	for (;;) {
		...
		spu = event.data1;
		int sret = spu_thread_printf(spu, (int)event.data3);
...
sampleSpursUtilSpuPrintfServiceInitialize(SampleUtilSpursPrintfService *service, CellSpurs* spurs, int prio)
{
	...
	ret = sys_ppu_thread_create (&service->spu_printf_handler, spu_printf_handler_entry,
								 (uint64_t)(uintptr_t)service, prio, SPURS_PRINTSERVICE_STACK_SIZE, SYS_PPU_THREAD_CREATE_JOINABLE, 
								 "spu_printf_handler");
	if (ret) {
